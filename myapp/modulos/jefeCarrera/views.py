        ...
        if request.method == 'POST':
            ...
            if form.is_valid():
                ...
                if tipoEvento == 'coordinadores':
                    ...
                    try:
                        created_event = service.events().insert(calendarId='primary', body=event).execute()
                        nuevoEvento = Evento.objects.create(fecha=fecha, summary= summary, location = location, start =start, end=end, 
...
def editEventosView (request, id_evento):
    ...
    if perfilTemp.rol_actual == 'JC':
        ...
        return render(request, 'jefeCarrera/editEvents.html',  {'status': status, 'summary': summary, 'start':start, 'end': end, 'location':location, 'descripcion':descripcion, 'fecha':fecha, 'form':form, 'evento': evento, 'username':request.user.username})
    else:
        return redirect ('/errorLogin/')
